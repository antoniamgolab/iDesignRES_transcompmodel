...
yaml_file_path = normpath(joinpath(@__DIR__, "data/calibration_iteration_11.yaml"))
println("Constructed file path: $yaml_file_path")

using Dates

timestamp = Dates.format(now(), "yyyy-mm-dd_HH-MM-SS")
case = "calibration_11_$timestamp"
...
constraint_constant_fueling_since_yinit(model, data_structures)
@info "Constraint for constant fueling since yinit created successfully"

constraint_fueling_infrastructure_init(model, data_structures)
@info "Constraint for fueling infrastructure init created successfully"

constraint_fueling_cap_constant(model, data_structures)
@info "Constraint for fueling capacity constant created successfully"
