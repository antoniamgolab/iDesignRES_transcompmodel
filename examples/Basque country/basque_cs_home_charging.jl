...
# Define list of YAML files to run for home charging scenarios
yaml_files = [
    "data/td_ONENODE_scenario_1_Low_20pct_home_charging_ex.yaml",
    "data/td_ONENODE_scenario_2_Medium-Low_40pct_home_charging_ex.yaml",
    "data/td_ONENODE_scenario_3_Medium_60pct_home_charging_ex.yaml",
    "data/td_ONENODE_scenario_4_Medium-High_80pct_home_charging_ex.yaml",
    "data/td_ONENODE_scenario_5_Base_100pct_home_charging_ex.yaml",
    "data/td_ONENODE_scenario_6_High_120pct_home_charging_ex.yaml",
    # Add more home charging scenario files here as needed
]
...
for (file_idx, yaml_file) in enumerate(yaml_files)
    println("\n" * "="^80)
    println("Processing home charging scenario $file_idx of $(length(yaml_files))")
    println("="^80)
    ...
    case = "cs_home_$(file_name)_$timestamp"
    ...
    if data_structures["detour_time_reduction_list"] != []
        ...
        # this is not in the problem - this was the OLD formulation!!
    ...
    set_optimizer_attribute(model, "FeasibilityTol", 1e-3) # or 1e-4
    ...
    @info "Model solved successfully for case: $case"

    println("\n" * "="^80)
    println("Completed scenario $file_idx of $(length(yaml_files)): $case")
    println("="^80 * "\n")
end

println("\n" * "="^80)
println("ALL HOME CHARGING SCENARIOS COMPLETED!")
println("Total scenarios processed: $(length(yaml_files))")
println("="^80)
