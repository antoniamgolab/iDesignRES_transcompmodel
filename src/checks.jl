...
function check_model_parametrization(data_dict::Dict, required_keys::Vector{String})
     check_required_keys(data_dict["Model"], required_keys)
end
...
"""
function check_required_sub_keys(data_dict::Dict, required_keys::Vector{String}, parent_key::String)
    for key ∈ required_keys
        for d in data_dict[parent_key]
...
function check_uniquness_of_ids(data_structures::Dict, required_keys)
    for key ∈ required_keys
        if key != "Model"
            ids = [d["id"] for d in data_structures[key]]
...
function check_correct_formats_GeographicElement(data_structures::Dict, years)
    for ij in 1:length(data_structures["GeographicElement"])
        ...
        @assert (isa(d["carbon_price"], Array{Float64, 1}) || isa(d["carbon_price"], Array{Int, 1})) "The key 'carbon_price' in 'GeographicElement' must be an array with int or float values. Error at $(d["id"])."
...
function check_correct_formats_FinancialStatus(data_structures::Dict)
    for ij in 1:length(data_structures["FinancialStatus"])
        ...
        @assert (isa(fs["monetary_budget_operational"], Float64) || isa(fs["monetary_budget_operational"], Int)) "The key 'monetary_budget_operational' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        @assert (isa(fs["monetary_budget_operational_lb"], Float64) || isa(fs["monetary_budget_operational_lb"], Int)) "The key 'monetary_budget_operational_lb' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        @assert (isa(fs["monetary_budget_operational_ub"], Float64) || isa(fs["monetary_budget_operational_ub"], Int)) "The key 'monetary_budget_operational_ub' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        @assert (isa(fs["monetary_budget_purchase"], Float64) || isa(fs["monetary_budget_purchase"], Int)) "The key 'monetary_budget_purchase' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        @assert (isa(fs["monetary_budget_purchase_lb"], Float64) || isa(fs["monetary_budget_purchase_lb"], Int)) "The key 'monetary_budget_purchase_lb' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        @assert (isa(fs["monetary_budget_purchase_ub"], Float64) || isa(fs["monetary_budget_purchase_ub"], Int)) "The key 'monetary_budget_purchase_ub' in 'FinancialStatus' must be a float or int value. Error at $(fs["id"])."
        ...
        if fs["monetary_budget_operational"] < fs["monetary_budget_operational_lb"]
            error("The lower bound of the operational monetary budget must be smaller than the operational monetary budget.")
        end

        if fs["monetary_budget_operational"] > fs["monetary_budget_operational_ub"]
            error("The upper bound of the operational monetary budget must be larger than the operational monetary budget.")
        end

        if fs["monetary_budget_operational_lb"] > fs["monetary_budget_operational_ub"]
            error("The upper bound of the operational monetary budget must be larger than the lower bound of the operational monetary budget.")
        end

        if fs["monetary_budget_purchase"] < fs["monetary_budget_purchase_lb"]
            error("The lower bound of the purchase monetary budget must be smaller than the purchase monetary budget.")
        end

        if fs["monetary_budget_purchase"] > fs["monetary_budget_purchase_ub"]
            error("The upper bound of the purchase monetary budget must be larger than the purchase monetary budget.")
        end

        if fs["monetary_budget_purchase_lb"] > fs["monetary_budget_purchase_ub"]
            error("The upper bound of the purchase monetary budget must be larger than the lower bound of the purchase monetary budget.")
        end
...
function check_correct_format_Mode(data_structures::Dict, years)
    for ij = 1:length(data_structures["Mode"])
        ...
        @assert (isa(m["costs_per_ukm"], Array{Float64, 1}) || isa(m["costs_per_ukm"], Array{Int, 1})) "The key 'costs_per_ukm' in 'Mode' must be an array with float or integer values. Error at $(m["id"])."
        @assert (isa(m["emission_factor"], Array{Float64, 1}) || isa(m["emission_factor"], Array{Int, 1})) "The key 'emission_factor' in 'Mode' must be an array with float or integer values. Error at $(m["id"])."
        @assert (isa(m["infrastructure_expansion_costs"], Array{Float64, 1}) || isa(m["infrastructure_expansion_costs"], Array{Int, 1})) "The key 'infrastructure_expansion_costs' in 'Mode' must be an array with float or integer values. Error at $(m["id"])."
        @assert (isa(m["infrastructure_om_costs"], Array{Float64, 1}) || isa(m["infrastructure_om_costs"], Array{Int, 1})) "The key 'infrastructure_om_costs' in 'Mode' must be an array with float or integer values. Error at $(m["id"])."
        @assert (isa(m["waiting_time"], Array{Float64, 1}) || isa(m["waiting_time"], Array{Int, 1})) "The key 'waiting_time' in 'Mode' must be an array with float or integer values. Error at $(m["id"])."
...
function check_correct_format_Fuel(data_structures::Dict, years::Int)
    for ij = 1:length(data_structures["Fuel"])
        ...
        @assert (isa(f["cost_per_kWh"], Array{Float64, 1}) || isa(f["cost_per_kWh"], Array{Int, 1})) "The key 'cost_per_kWh' in 'Fuel' must be an array with float or integer values. Error at $(f["id"])."
        @assert (isa(f["cost_per_kW"], Array{Float64, 1}) || isa(f["cost_per_kW"], Array{Int, 1})) "The key 'cost_per_kW' in 'Fuel' must be an array with float or integer values. Error at $(f["id"])."
        @assert (isa(f["emission_factor"], Array{Float64, 1}) || isa(f["emission_factor"], Array{Int, 1})) "The key 'emission_factor' in 'Fuel' must be an array with float or integer values. Error at $(f["id"])."
        @assert (isa(f["fueling_infrastructure_om_costs"], Array{Float64, 1}) || isa(f["fueling_infrastructure_om_costs"], Array{Int, 1})) "The key 'fueling_infrastructure_om_costs' in 'Fuel' must be an array with float or integer values. Error at $(f["id"])."
...
function check_correct_format_Regiontype(data_structures::Dict, years::Int)
    for ij in 1:length(data_structures["Regiontype"])
        ...
        @assert (isa(rt["costs_var"], Array{Float64, 1}) || isa(rt["costs_var"], Array{Int, 1})) "The key 'costs_var' in 'Regiontype' must be an array with float or integer values. Error at $(rt["id"])."
        @assert (isa(rt["costs_fix"], Array{Float64, 1}) || isa(rt["costs_fix"], Array{Int, 1})) "The key 'costs_fix' in 'Regiontype' must be an array with float or integer values. Error at $(rt["id"])."
...
function check_correct_format_TechVehicle(data_structures::Dict, years::Int, generations::Int)
    for ij in 1:length(data_structures["TechVehicle"])
        ...
        @assert isa(tv["capital_cost"], Array{Float64, 1}) || isa(tv["capital_cost"], Array{Int, 1}) "The key 'capital_cost' in 'TechVehicle' must be an array with float or integer values. Error at $(tv["id"])."
        @assert (isa(tv["maintnanace_cost_annual"], Array{Array{Float64, 1}, 1}) || isa(tv["maintnanace_cost_annual"], Array{Array{Int, 1}, 1})) "The key 'maintnanace_cost_annual' in 'TechVehicle' must be a two-dimensional array with float or integer values. Error at $(tv["id"])."
        @assert (isa(tv["maintnance_cost_distance"],Array{Array{Float64, 1}, 1}) || isa(tv["maintnance_cost_distance"], Array{Array{Int, 1}, 1})) "The key 'maintenance_cost_distance' in 'TechVehicle' must be a two-dimensional array with float or integer values. Error at $(tv["id"])." 
        ...
        @assert (isa(tv["spec_cons"], Array{Float64, 1}) || isa(tv["spec_cons"], Array{Int, 1})) "The key 'spec_cons' in 'TechVehicle' must be an array with float or integer values. Error at $(tv["id"])."
        ...
        @assert (isa(tv["AnnualRange"], Array{Float64, 1}) || isa(tv["AnnualRange"], Array{Int, 1})) "The key 'AnnualRange' in 'TechVehicle' must be an array with float or integer values. Error at $(tv["id"])."
        @assert isa(tv["products"], Array{String, 1}) "The key 'products' in 'TechVehicle' must be an array of string values. Error at $(tv["id"])."
        @assert isa(tv["battery_capacity"], Array{Float64, 1}) || isa(tv["battery_capacity"], Array{Int, 1}) "The key 'battery_capacity' in 'TechVehicle' must be an array with float or integer values. Error at $(tv["id"])."
        @assert isa(tv["peak_charging"], Array{Float64, 1}) || isa(tv["peak_charging"], Array{Int, 1}) "The key 'peak_charging' in 'TechVehicle' must be an array with float or integer values. Error at $(tv["id"])."
        ...
        for i in 1:length(tv["maintnance_cost_distance"])
        ...
        for i in 1:length(tv["maintnanace_cost_annual"])
        ...
        @assert length(tv["battery_capacity"]) == generations "The key 'battery_capacity' in 'TechVehicle' must have the same length as the years of the optimization horizon. Error at $(tv["id"])."
        @assert length(tv["peak_charging"]) == generations "The key 'peak_charging' in 'TechVehicle' must have the same length as the years of the optimization horizon. Error at $(tv["id"])."
...
function check_correct_format_InitialVehicleStock(data_structures::Dict, y_init::Int, g_init::Int)
    for ij in 1:length(data_structures["InitialVehicleStock"])
        ...
        if ivs["year_of_purchase"] < g_init
            error("The year of purchase must not be earlier than the year of the first considered generation. Error at $(ivs["id"]).")
        end
...
function check_correct_format_InitialFuelingInfr(data_structures::Dict)
    for ij in 1:length(data_structures["InitialFuelingInfr"])
...
function check_correct_format_InitialModeInfr(data_structures::Dict)
    for ij in 1:length(data_structures["InitialModeInfr"])
...
function check_correct_format_Odpair(data_structures::Dict, years::Int)
    for ij in 1:length(data_structures["Odpair"])
...
function check_correct_format_Speed(data_structures::Dict)
    for ij in 1:length(data_structures["Speed"])
