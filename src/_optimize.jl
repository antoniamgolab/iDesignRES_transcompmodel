...
L_node_edges = Dict(
    (1, 1) => 200, (2, 1) => 200, (3, 1) => 200, (4, 1) => 5000, (5, 1) => 5000
) # (e/n)k; route length within a note 
...
@constraint(
    model,
    [subset in yr_set],
    sum(f[(subset[1], k, t, g)] for k in K_r[subset[2]] for g in g_vec for t in t_vec) ==
        F[(subset[1], subset[2])]
)
...
@constraint(
    ...
    h[subset] >= sum(
        (
            sum(L_node_edges[(route_element, k)] for route_element in route[k]) /
            (W[(subset[3], subset[4])] * L_a[(subset[3], subset[4])])
        ) * f[(subset[1], k, subset[3], subset[4])] for k in K_r[subset[2]]
    )
)
# t = 1
@constraint(
...
@objective(
    model,
    Min,
    sum(c_transport[subset] * f[subset] for subset in yktg_set) +
        sum(h_plus[subset] * c_h[(subset[1], subset[3], subset[4])] for subset in yrtg_set)
)
