...
@constraint(model, [subset in yr_set], sum(f[(subset[1], k, t, g)] for k in K_r[subset[2]] for g in g_vec for t in t_vec) == F[(subset[1], subset[2])])

# ----
# vehicle fleet constraints
# ---
@constraint(model, [subset in yrtg_set], h[subset] >= sum((sum(L_node_edges[(route_element, k)] for route_element in route[k])/(W[(subset[3], subset[4])] * L_a[(subset[3], subset[4])])) * f[(subset[1], k, subset[3], subset[4])] for k in K_r[subset[2]]))
# t = 1
@constraint(model, [subset in rtg_set], h[(1, subset[1], subset[2], subset[3])] == h_init[subset] + h_plus[(1, subset[1], subset[2], subset[3])] - h_minus[(1, subset[1], subset[2], subset[3])])
# t > 1
@constraint(model, [subset in later_yrtg_set], h[subset] == h[(subset[1]-1, subset[2], subset[3], subset[4])] + h_plus[subset] - h_minus[subset])

# ---
# objective
# ---
@objective(model, Min, sum(c_transport[subset]*f[subset] for subset in yktg_set) + sum(h_plus[subset]*c_h[(subset[1], subset[3], subset[4])] for subset in yrtg_set))
